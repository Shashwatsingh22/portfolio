...
function buildResume(d) {
  return `
    ...
    ${buildSection('Blogs', d.blogs.map(buildBlog).join(''))}
...
function buildBlog(b) {
  return `<div class="blog-item"><a href="${b.url}" target="_blank"><strong>${b.title}</strong></a> — ${b.gist}</div>`;
}
